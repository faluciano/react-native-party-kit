...
export function useGameClient<S extends IGameState, A extends IAction>(config: ClientConfig<S, A>) {
  const [status, setStatus] = useState<'connecting' | 'connected' | 'disconnected' | 'error'>('disconnected');
  const [playerId, setPlayerId] = useState<string | null>(null);
  
  // Local Optimistic State
  const [state, dispatchLocal] = useReducer(config.reducer, config.initialState);
  ...
  const connect = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      if (reconnectAttempts.current < MAX_RETRIES) {
        const delay = Math.min(BASE_DELAY * Math.pow(2, reconnectAttempts.current), 10000);
        reconnectAttempts.current++;
        
        if (config.debug) console.log(`[GameClient] Reconnecting in ${delay}ms...`);
